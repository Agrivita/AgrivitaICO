...
contract Agricoin is Owned
{
    ...
    // Get dividends.
    event GetDividends(address indexed inestor, uint etherAmount);

    // Constructor.
    function Agricoin(uint payout_period_start, uint payout_period_end, address[] payerList) public
    {
    ...
    function getUnpaid() onlyPayer() external returns (bool)
    {
    ...
    function recalculate(address user) internal returns (bool)
    {
        if (isSuccessfulIco)
        {
            if (balances[user].icoBalance != 0)
            {
                ...
                totalSupply += balances[user].icoBalance;
